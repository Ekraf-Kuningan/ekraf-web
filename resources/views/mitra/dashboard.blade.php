...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5 mb-8">
            <!-- Active -->
            <div
                class="group rounded-2xl bg-white shadow-md hover:shadow-xl border border-gray-100 p-6 h-32 md:h-36 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1">
                <div>
            ...
            <!-- Rejected -->
            <div
                class="group rounded-2xl bg-white shadow-md hover:shadow-xl border border-gray-100 p-6 h-32 md:h-36 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 sm:col-span-2 lg:col-span-1">
                <div>
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg md:text-xl font-bold text-gray-900">Kategori</h3>
                <a href="{{ route('mitra.products') }}"
        ...
        <div class="mb-12">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg md:text-xl font-bold text-gray-900">Produk Saya</h3>
                <a href="{{ route('mitra.products') }}"
                    class="text-sm md:text-base text-blue-600 hover:text-blue-700 font-medium transition-colors">Lihat
                    Semua →</a>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-5">
                @forelse($products as $p)
                    <div
                        class="group rounded-2xl bg-white shadow-md hover:shadow-xl overflow-hidden transition-all duration-300 border border-gray-100">
                        <!-- Product Image -->
                        <div class="aspect-square bg-gray-100 overflow-hidden relative">
                            ...
                            <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100">
                                <div class="flex gap-2">
                                    ...
                                    <button onclick="event.stopPropagation(); openProductModal('{{ $p->id }}')" 
                                       class="bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-lg transition-colors shadow-lg"
                                       title="Detail Produk">
                                    ...
                                    <a href="{{ route('mitra.products.edit', $p->id) }}" 
                                       class="bg-orange-500 hover:bg-orange-600 text-white p-2 rounded-lg transition-colors shadow-lg"
                                       title="Edit Produk">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    
                                    <!-- Delete Button -->
                                    <button onclick="confirmDelete('{{ $p->id }}', '{{ $p->name }}')" 
                            ...
                            <div class="absolute top-2 right-2">
                                @if($p->status === 'approved')
                                    <span class="bg-green-500 text-white text-xs px-2 py-1 rounded-full font-semibold shadow">
                                        <i class="fas fa-check-circle"></i> Aktif
                                    </span>
                                @elseif($p->status === 'pending')
                                    <span class="bg-yellow-500 text-white text-xs px-2 py-1 rounded-full font-semibold shadow">
                                ...
                                @else
                                    <span class="bg-gray-500 text-white text-xs px-2 py-1 rounded-full font-semibold shadow">
                        ...
                        <!-- Product Info -->
                        <div class="p-4">
                            <div class="text-sm font-semibold text-gray-900 line-clamp-2 mb-2 min-h-[2.5rem]">
                                {{ $p->name }}
                            </div>
                            <div class="text-orange-600 font-bold text-base mb-3">
                            ...
                            <div class="flex gap-1">
                                <button onclick="event.stopPropagation(); openProductModal('{{ $p->id }}')" 
                                   class="flex-1 bg-blue-500 hover:bg-blue-600 text-white text-xs py-2 px-2 rounded-lg transition-colors text-center">
                                    <i class="fas fa-eye"></i> <span class="hidden sm:inline">Detail</span>
                                </button>
                                <a href="{{ route('mitra.products.edit', $p->id) }}" 
                                   class="flex-1 bg-orange-500 hover:bg-orange-600 text-white text-xs py-2 px-2 rounded-lg transition-colors text-center">
                                    <i class="fas fa-edit"></i> <span class="hidden sm:inline">Edit</span>
                                </a>
                                <button onclick="confirmDelete('{{ $p->id }}', '{{ $p->name }}')" 
                                        class="flex-1 bg-red-500 hover:bg-red-600 text-white text-xs py-2 px-2 rounded-lg transition-colors">
                ...
                @empty
                    <div class="col-span-full rounded-2xl bg-white shadow-md border border-gray-100 p-12 text-center">
                        ...
                        <p class="text-gray-600 font-medium mb-3">Belum ada produk</p>
                        <p class="text-gray-500 text-sm mb-6">Mulai dengan menambah produk pertama Anda</p>
                        <a href="{{ route('mitra.products.create') }}"
                            class="inline-block px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-xl transition-colors shadow-md hover:shadow-lg">
                            Tambah Produk
                        </a>
                    </div>
                @endforelse
        ...
        <div class="mb-12">
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-600">
                        Jelajahi produk dari mitra EKRAF lainnya
                        <span class="inline-flex items-center ml-2 px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-semibold rounded-full">
                            <i class="fas fa-eye mr-1"></i>
                            Hanya Lihat
                        </span>
                    </p>
                </div>
                <a href="{{ route('mitra.katalog') }}"
                    class="text-sm md:text-base text-orange-600 hover:text-orange-700 font-semibold transition-colors flex items-center">
                    Lihat Semua
            ...
            @if ($katalogsBySubSektor->count() > 0)
                @foreach ($katalogsBySubSektor as $subSektor)
                    @if ($subSektor->katalogs->count() > 0)
                        <div class="mb-8">
                            <!-- Sub Sektor Header -->
                            <div class="flex items-center justify-between mb-4">
                                <h4 class="text-base md:text-lg font-bold text-gray-800 flex items-center">
                                    <span class="w-1 h-6 bg-orange-500 rounded-full mr-3"></span>
                                    {{ $subSektor->title }}
                                </h4>
                                <a href="{{ route('mitra.katalog', ['sub_sektor' => $subSektor->id]) }}"
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
                                @foreach ($subSektor->katalogs as $katalog)
                                    <a href="{{ route('katalog.show', $katalog->slug) }}" class="group block">
                                        <div
                                            class="rounded-2xl bg-white shadow-md hover:shadow-xl overflow-hidden transition-all duration-300 hover:-translate-y-1 border border-gray-100 hover:border-orange-200">
                                            <!-- Katalog Image -->
                                            <div class="aspect-square bg-gray-100 overflow-hidden relative">
                                                <img src="{{ $katalog->image_url }}" alt="{{ $katalog->title }}"
                                                    class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                                                ...
                                                <div class="absolute top-2 left-2">
                                                    <span
                                                        class="px-2 py-1 bg-orange-500 text-white text-xs font-semibold rounded-full shadow-lg">
                                                        {{ $subSektor->title }}
                                                    </span>
                                                </div>
                                                ...
                                                <div class="absolute top-2 right-2">
                                                    <span
                                                        class="px-2 py-1 bg-black bg-opacity-60 text-white text-xs font-semibold rounded-full shadow-lg">
                                            ...
                                            <div class="p-4">
                                                <h5
                                                    class="text-sm font-semibold text-gray-900 line-clamp-2 mb-2 min-h-[2.5rem] group-hover:text-orange-600 transition-colors">
                                                    {{ $katalog->title }}
                                                </h5>

                                                <p class="text-xs text-gray-600 mb-2 line-clamp-2">
                                                    {{ Str::limit(strip_tags($katalog->content), 60) }}
                                                </p>
                ...
                <div class="text-center mt-8">
                    <a href="{{ route('mitra.katalog') }}"
                        class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-semibold rounded-xl hover:from-orange-600 hover:to-orange-700 transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-105">
                        <i class="fas fa-store mr-2"></i>
                        Jelajahi Semua Katalog Produk
                        <i class="fas fa-arrow-right ml-2"></i>
                    </a>
                </div> --}}
            @else
                <!-- Empty State -->
                <div class="rounded-2xl bg-white shadow-md border border-gray-100 p-12 text-center">
                    <div class="w-20 h-20 bg-orange-50 rounded-full mx-auto mb-4 flex items-center justify-center">
                        <i class="fas fa-store text-4xl text-orange-400"></i>
                    </div>
                    <p class="text-gray-600 font-medium mb-2">Belum Ada Produk dari Mitra Lain</p>
                    <p class="text-gray-500 text-sm mb-6">Katalog produk akan muncul ketika ada mitra lain yang telah
                        mengunggah produk</p>
                    <a href="{{ route('mitra.katalog') }}"
                        class="inline-flex items-center px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-xl transition-colors shadow-md hover:shadow-lg">
                        <i class="fas fa-store mr-2"></i>
                        Lihat Katalog
                    </a>
                </div>
    ...
    <div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 transform transition-all">
            <div class="text-center mb-6">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-2">Konfirmasi Hapus Produk</h3>
                <p class="text-gray-600">Apakah Anda yakin ingin menghapus produk <span id="productName" class="font-semibold text-gray-900"></span>?</p>
            ...
            <form id="deleteForm" method="POST" class="space-y-3">
                ...
                <div class="flex gap-3">
                    <button type="button" onclick="closeDeleteModal()" 
                            class="flex-1 px-4 py-3 bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold rounded-xl transition-colors">
                        Batal
                    </button>
                    <button type="submit" 
                            class="flex-1 px-4 py-3 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-xl transition-colors shadow-md hover:shadow-lg">
...
<script>
    ...
    function confirmDelete(productId, productName) {
        ...
        document.getElementById('deleteForm').action = `/mitra/products/${productId}`;
