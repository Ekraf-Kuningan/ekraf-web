        ...
        <div class="mb-6">
            <div class="rounded-2xl bg-gradient-to-r from-orange-400 to-orange-500 p-6 md:p-8 text-white shadow-lg">
                <p class="text-sm md:text-base opacity-90 mb-2">Selamat
                    datang{{ isset($pelakuEkraf->business_name) ? ', ' . $pelakuEkraf->business_name : '' }}</p>
                    ...
                    @if ($pelakuEkraf)
                        <div class="space-y-3">
                            <div>
                                ...
                                <p class="font-semibold text-gray-900">{{ $pelakuEkraf->business_name ?? '-' }}</p>
                            </div>
                            <div>
                                ...
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        ...
                                        @if($notif->status === 'rejected' && $notif->rejection_reason)
                                            <div class="mt-2 p-2 bg-white rounded-lg border border-red-200">
                                                <p class="text-xs text-gray-600 font-medium mb-1">
                                                    <i class="fas fa-info-circle mr-1 text-red-500"></i>Alasan Penolakan:
                                                </p>
                                                <p class="text-xs text-gray-700">{{ $notif->rejection_reason }}</p>
                                            </div>
                                        @endif
                                    </div>
                                    <span
            ...
            <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    @forelse($latestProducts as $product)
                            ...
                            <div class="flex-shrink-0 ml-3">
                                ...
                                @elseif($product->status === 'rejected')
                                    <span class="px-3 py-1 bg-red-100 text-red-700 text-xs font-semibold rounded-full" 
                                          title="{{ $product->rejection_reason ? 'Alasan: ' . $product->rejection_reason : 'Produk Ditolak' }}">
                                        <i class="fas fa-times-circle mr-1"></i>
                                        Ditolak
                                    </span>
                                @else
                        ...
                        <div class="mt-2 ml-20 p-2 bg-red-50 border border-red-200 rounded-lg">
                            <p class="text-xs text-red-700">
                                <i class="fas fa-info-circle mr-1"></i>
                                <strong>Alasan:</strong> {{ $product->rejection_reason }}
                            </p>
                        </div>
                        @endif
                    @empty
                ...
                <div class="md:hidden overflow-x-auto -mx-4 pl-4 pr-4 pb-2">
                    <div class="flex gap-3">
                        @foreach ($categories as $c)
                            <a href="{{ route('pelaku-ekraf.products', ['category' => $c->id]) }}"
