...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 mb-6">
        <form method="GET" action="{{ route('pelaku-ekraf.katalog') }}" class="space-y-4">
            <!-- Search -->
            <div class="flex-1">
                <div class="relative">
            ...
            <div class="flex gap-2">
                ...
                @if(request('search') || request('sub_sektor'))
                    <a 
                        href="{{ route('pelaku-ekraf.katalog') }}" 
        ...
        <div class="mb-6 flex flex-wrap items-center gap-2">
            ...
            @if($selectedSubSektor)
                <span class="inline-flex items-center px-3 py-1 rounded-full bg-orange-100 text-orange-700 text-sm">
                    ...
                    <a href="{{ route('pelaku-ekraf.katalog', ['search' => request('search')]) }}" class="ml-2 hover:text-orange-900">
            ...
            @if(request('search'))
                <span class="inline-flex items-center px-3 py-1 rounded-full bg-blue-100 text-blue-700 text-sm">
                    ...
                    "{{ request('search') }}"
                    <a href="{{ route('pelaku-ekraf.katalog', ['sub_sektor' => request('sub_sektor')]) }}" class="ml-2 hover:text-blue-900">
                        <i class="fas fa-times"></i>
                    </a>
                </span>
            @endif
    ...
    @else
        ...
        <div class="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            <a href="{{ route('pelaku-ekraf.katalog') }}" class="inline-block px-6 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition">
                <i class="fas fa-redo mr-2"></i>
                Lihat Semua Katalog
            </a>
        </div>
        ...
        <div class="mb-6 flex flex-wrap items-center gap-2">
            ...
            @if($selectedSubSektor)
                <span class="inline-flex items-center px-3 py-1 rounded-full bg-orange-100 text-orange-700 text-sm">
                    <i class="fas fa-tag mr-2"></i>
                    {{ $selectedSubSektor->title }}
                    <a href="{{ route('pelaku-ekraf.katalog', ['search' => request('search'), 'my_products' => request('my_products')]) }}" class="ml-2 hover:text-orange-900">
                        <i class="fas fa-times"></i>
                    </a>
                </span>
            @endif
            @if(request('my_products'))
                <span class="inline-flex items-center px-3 py-1 rounded-full bg-purple-100 text-purple-700 text-sm">
                    ...
                    Katalog Saya
                    <a href="{{ route('pelaku-ekraf.katalog', ['search' => request('search'), 'sub_sektor' => request('sub_sektor')]) }}" class="ml-2 hover:text-purple-900">
                        <i class="fas fa-times"></i>
                    </a>
                </span>
            @endif
            @if(request('search'))
                <span class="inline-flex items-center px-3 py-1 rounded-full bg-blue-100 text-blue-700 text-sm">
                    <i class="fas fa-search mr-2"></i>
                    "{{ request('search') }}"
                    <a href="{{ route('pelaku-ekraf.katalog', ['sub_sektor' => request('sub_sektor'), 'my_products' => request('my_products')]) }}" class="ml-2 hover:text-blue-900">
                        <i class="fas fa-times"></i>
                    </a>
                </span>
            @endif
            ...
            @foreach($products as $product)
                <div class="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group">
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex gap-2">
                            <a 
                                href="{{ route('pelaku-ekraf.katalog.show', $product->id) }}" 
        ...
        <div class="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            <p class="text-gray-600 mb-4">
                @if(request('my_products'))
                    ...
                    @else
                        Anda belum memiliki produk. <a href="{{ route('pelaku-ekraf.products.create') }}" class="text-orange-600 hover:underline">Tambah produk sekarang</a>
                    @endif
