...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Header -->
        <div class="mb-8">
            <a href="{{ route('pelaku-ekraf.dashboard') }}" class="inline-flex items-center text-sm text-gray-600 hover:text-orange-600 mb-4 transition-colors">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
                    ...
                    <!-- Delete Photo Button -->
                    @if($user->image)
                    <form action="{{ route('pelaku-ekraf.profile.deleteImage') }}" method="POST" class="mb-4" onsubmit="return confirm('Yakin ingin menghapus foto profil?')">
                        @csrf
                        @method('DELETE')
                    ...
                    <div class="pt-4 border-t border-gray-200">
                        ...
                        <div class="space-y-2 text-sm">
                            <p class="text-gray-600">
                                ...
                                {{ $pelakuEkraf->business_name ?? '-' }}
                            </p>
                            <p class="text-gray-600">
                                <span class="font-medium">Sub Sektor:</span><br>
                                {{ $pelakuEkraf->subSektor->title ?? '-' }}
                            </p>
                            <p class="text-gray-600">
                                <span class="font-medium">Status:</span><br>
                                <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full
                                    @if($pelakuEkraf->status === 'active') bg-green-100 text-green-800
                                    @elseif($pelakuEkraf->status === 'pending') bg-yellow-100 text-yellow-800
                                    @else bg-gray-100 text-gray-800 @endif">
                                    {{ ucfirst($pelakuEkraf->status) }}
                                </span>
                            </p>
                        </div>
                    </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <form action="{{ route('pelaku-ekraf.profile.update') }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8">
                        @csrf
                        ...
                        <div class="mt-8 flex items-center justify-end space-x-4">
                            <a href="{{ route('pelaku-ekraf.dashboard') }}" class="px-6 py-3 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-colors font-medium">
                                Batal
                            </a>
                            <button 
                                type="submit" 
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <form action="{{ route('pelaku-ekraf.profile.updatePassword') }}" method="POST" class="p-6 md:p-8">
                        @csrf
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <form action="{{ route('pelaku-ekraf.testimonial.store') }}" method="POST">
                            @csrf
                        ...
                        @if($testimonial)
                            <form id="delete-testimonial-form" action="{{ route('pelaku-ekraf.testimonial.destroy') }}" method="POST" class="hidden">
                                @csrf
                                @method('DELETE')
                            </form>
                        @endif
